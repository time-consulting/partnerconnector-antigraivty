...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/deals', requireAuth, async (req: any, res) => {
    try {
      ...
      // Auto-generate funding quote for business_funding + existing Dojo customers
      const isAutoQuoteEligible = (
        deal.productType === 'business_funding' &&
        deal.takesCardPayments === true &&
        deal.currentCardProvider === 'dojo'
      );

      if (isAutoQuoteEligible) {
        try {
          // Build funding quote data (same structure as admin generate-funding-quote endpoint)
          const fundingQuoteData: any = {
            quoteType: 'business_funding',
            isDojo: true,
            takesCardPayments: true,
            currentCardProvider: 'dojo',
            ...
            instructions: {
              ...
              message: 'As an existing Dojo customer, you can apply directly for business funding. Complete the application form below to request your funding quotes.',
              steps: [
                'Complete the application form to request funding',
                'You will receive 3 different funding quotes — this is a soft search only and will not affect your credit score',
                'At this stage you can still choose not to proceed if the funding amounts don\'t suit you',
                'A specialised funding team will contact you directly with your quotes',
                'If you choose to proceed, your application will go straight to approved or declined — no further credit checks needed'
              ],
              ctaLabel: 'Proceed with Application',
              ctaAction: 'proceed_to_application',
              softSearchNote: 'This is a soft search only — it will not appear on your credit file.',
            },
          };

          // Create a quote record (same as admin endpoint, all rates zeroed for funding)
          const autoQuote = await storage.createQuote({
            referralId: deal.id,
            creditCardRate: 0,
            debitCardRate: 0,
            corporateCardRate: 0,
            visaBusinessDebitRate: 0,
            otherBusinessDebitRate: 0,
            amexRate: 0,
            secureTransactionFee: 0,
            ...
            sentAt: new Date(),
            createdBy: userId,
            adminNotes: JSON.stringify(fundingQuoteData),
          });

          // Update deal to quote_sent stage — try with all fields, fall back without quoteType/quoteData
          const autoQuoteAdminNotes = `[${new Date().toLocaleString()}] Auto-generated funding quote for existing Dojo customer. Quote ID: ${autoQuote.id}. Funding Amount: ${deal.fundingAmount || 'not specified'}.`;
          ...
          try {
            await storage.updateDeal(deal.id, {
              ...
              quoteGenerated: true,
              quoteType: 'business_funding',
              quoteData: fundingQuoteData,
              adminNotes: autoQuoteAdminNotes,
            });
          } catch (updateErr: any) {
            console.warn('Auto-quote: full updateDeal failed, falling back to minimal update:', updateErr?.message);
            await storage.updateDeal(deal.id, {
              dealStage: 'quote_sent',
              quoteGenerated: true,
              adminNotes: autoQuoteAdminNotes,
          ...
          // Update the deal object so the client response reflects the auto-quote
          deal.dealStage = 'quote_sent';
          deal.quoteGenerated = true;
          deal.quoteType = 'business_funding';
          deal.quoteData = fundingQuoteData;

          // Notify admins that this deal was auto-quoted
          const admins = await storage.getAdminUsers();
          for (const admin of admins) {
            await createNotificationForUser(admin.id, {
              type: 'status_update',
              title: 'Auto-Quote Generated (Dojo Funding)',
              message: `Funding quote auto-generated for ${deal.businessName} (${deal.dealId}). Existing Dojo customer — awaiting partner application.`,
              dealId: deal.id,
              businessName: deal.businessName,
            });
          }

          // Notify the partner that their funding quote is ready
          await createNotificationForUser(userId, {
            type: 'status_update',
            title: 'Funding Quote Ready',
            message: `Your funding quote for ${deal.businessName} (${deal.dealId}) is ready. Complete the application to proceed with your funding request.`,
            dealId: deal.id,
            businessName: deal.businessName,
          });

          console.log(`Auto-quote generated for Dojo funding deal: ${deal.dealId}, Quote: ${autoQuote.id}`);
        } catch (autoQuoteError) {
          // Non-fatal: if auto-quote fails, deal stays at quote_request_received
          // Admin can still manually generate the quote
          console.error(`Auto-quote failed for deal ${deal.id}:`, autoQuoteError);
        }
      }
  ...
  // Generate a business funding quote (instruction-based or with card payments)
  app.post('/api/admin/deals/:id/generate-funding-quote', requireAuth, requireAdmin, auditAdminAction('generate_funding_quote', 'deal'), async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      const isDojo = deal.currentCardProvider === 'dojo';
      const takesCards = deal.takesCardPayments === true;

      // Determine quote type
      let quoteType = 'business_funding';
      if (takesCards && !isDojo) {
        quoteType = 'funding_with_cards';  // Non-Dojo: needs card payments + funding
      }

      // Build funding quote data
      const fundingQuoteData: any = {
        quoteType,
        isDojo,
        takesCardPayments: takesCards,
        currentCardProvider: deal.currentCardProvider,
        fundingAmount: deal.fundingAmount,
        businessName: deal.businessName,
        businessEmail: deal.businessEmail,
        generatedAt: new Date().toISOString(),
        ...
        instructions: isDojo
          ...
          : {
            title: 'Card Payments & Business Funding',
            message: 'To access business funding, you\'ll first need to set up Dojo card payments. Once you\'re trading, you can apply for business funding through open banking.',
            steps: [
              'Review and approve the card payment quote below',
              'Complete the Dojo card payments application',
              'Once you\'re live and trading, we\'ll send you an open banking link',
              'Open banking verification will unlock your funding options'
            ],
            ctaLabel: 'Approve Quote & Apply',
            ctaAction: 'approve_and_apply',
            fundingNote: 'Business funding will be available once your Dojo account is live and trading.',
          },
      };

      // Create a quote record (for pipeline progression)
      // Store funding instructions in the quote's adminNotes so they're accessible even without quoteData column
      const quote = await storage.createQuote({
        referralId: id,
        creditCardRate: 0,
        debitCardRate: 0,
      ...
      // Update deal — try with all fields first, fall back to minimal update if quoteType/quoteData columns don't exist yet
      const adminNotesUpdate = (deal.adminNotes || '') + `\n[${new Date().toLocaleString()}] Funding quote generated by ${req.user.email}. Quote ID: ${quote.id}. Type: ${quoteType}. Provider: ${deal.currentCardProvider || 'none'}. Funding Amount: ${deal.fundingAmount || 'not specified'}.`;
      ...
      try {
        ...
        // If quoteType/quoteData columns don't exist yet (migration not run), fall back to minimal update
        console.warn('Full updateDeal failed (quoteType/quoteData columns may not exist yet), falling back to minimal update:', updateErr?.message);
        await storage.updateDeal(id, {
          dealStage: 'quote_sent',
          quoteGenerated: true,
          adminNotes: adminNotesUpdate,
      ...
      console.log('Funding quote generated for deal:', id, {
        admin: req.user.email,
        quoteId: quote.id,
        quoteType,
        isDojo,
      ...
      res.json({
        ...
        quoteId: quote.id,
        quoteType,
        message: `Funding quote sent to ${deal.businessEmail}`,
        quoteData: fundingQuoteData
      });
    } catch (error: any) {
      console.error("Error generating funding quote:", error);
      res.status(500).json({ message: error?.message || "Failed to generate funding quote" });
    }
  });
