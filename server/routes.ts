...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (isAutoQuoteEligible) {
        try {
          ...
          // Update deal to quote_sent stage with funding quote data
          await storage.updateDeal(deal.id, {
            dealStage: 'quote_sent',
            quoteGenerated: true,
            quoteType: 'business_funding',
            quoteData: fundingQuoteData,
            adminNotes: `[${new Date().toLocaleString()}] Auto-generated funding quote for existing Dojo customer. Quote ID: ${autoQuote.id}. Funding Amount: ${deal.fundingAmount || 'not specified'}.`,
          });
  ...
  app.post('/api/admin/deals/:id/generate-funding-quote', requireAuth, requireAdmin, auditAdminAction('generate_funding_quote', 'deal'), async (req: any, res) => {
    try {
      ...
      // Update deal with funding quote data and move to quote_sent
      await storage.updateDeal(id, {
        dealStage: 'quote_sent',
        quoteGenerated: true,
        quoteType,
      ...
      res.status(500).json({ message: "Failed to generate funding quote" });
    }
  });
